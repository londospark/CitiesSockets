...
public class TCPTestServer : MonoBehaviour
{
    ...
    public bool IsConnected => tcpListenerThread != null && tcpListenerThread.IsAlive;
    ...
    private void HandleClientWorker(object token)
    {
        ...
        using (TcpClient client = token as TcpClient)
        {
            ...
            var connectedClient = new ConnectedClient(data, client);
            ...
            try
            {
                using (var stream = client.GetStream())
                {
                    ...
                    while (stream.CanRead && (length = stream.Read(bytes, 0, bytes.Length)) != 0)
                    {
                        ...
                        var serverMessage = new ServerMessage(data, clientMessage);
    ...
    private void ProcessMessage(ConnectedClient connectedClient, string command)
    {
        ...
        switch (split[0].Trim())
        {
            ...
            case "!traffic":
                ...
                response = $"Your traffic is {trafficFlow}%";
    ...
    private bool SendMessage(TcpClient client, ServerMessage serverMessage)
    {
        if (client?.Connected == true)
        {
            try
            {
                ...
                if (stream.CanWrite)
                {
                    var serverMessageAsByteArray = Encoding.ASCII.GetBytes(JsonUtility.ToJson(serverMessage));
